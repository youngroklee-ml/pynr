# Dictionary, named list, environment
...
A. 원소가 생성된 순서는 의미가 없다.
B. 얼마나 많은 원소가 생성될 지 미리 알 수 없으며, 중간에 원소가 삭제될 수도 있는데, 이 때 처리 속도가 빨라야 한다.
C. 각 원소는 자주 읽히기 때문에, 자료를 찾는 속도가 빨라야 한다.
